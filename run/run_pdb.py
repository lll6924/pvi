...
@click.command()
@click.option('--model', default='Golf', help='The model name, must match a class name under jax_posteriordb/model/')
...
def main(model, objective, regularizer, posterior, seed, lamb, alpha, n, s, iterations, prediction_sample, learning_rate, optimizer):
    ...
    p = getattr(module2, posterior)(m.n)
    params = p.gen_params()
    ...
    os.makedirs(f"pdb_result/{model}_{alpha}_{n}_{posterior}_{objective}/", exist_ok=True)
    result_file = f'pdb_result/{model}_{alpha}_{n}_{posterior}_{objective}/{regularizer}_{lamb}_{seed}_{s}'
